	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			...
			function start()
			{
				...
				{
					// Workaround for Nomad origin (because they are allied to Nomad faction)
					nomadFaction = this.World.FactionManager.getFactionOfType(::Const.FactionType.Bandits);
					banner = ::Const.NomadBanners[::Math.rand(0,::Const.NomadBanners.len()-1)];
				}
				else
				{
					...
					nomadFaction = this.World.FactionManager.getFaction(nearest_nomads.getFaction());
					banner = nearest_nomads.getBanner();
				}
				local tile = this.Contract.getTileToSpawnLocation(this.Contract.m.Home.getTile(), 9, 15);
				local party = nomadFaction.spawnEntity(tile, "Indebted", false, this.Const.World.Spawn.NomadRaiders, 110 * this.Contract.getDifficultyMult() * this.Contract.getScaledDifficultyMult());
		...
		this.m.Screens.push({
			...
			function start()
			{
				...
				local tile = this.Contract.getTileToSpawnLocation(this.Contract.m.Home.getTile(), 9, 15);
				local party = nomadFaction.spawnEntity(tile, "Indebted", false, this.Const.World.Spawn.Slaves, 90 * this.Contract.getDifficultyMult() * this.Contract.getScaledDifficultyMult());
