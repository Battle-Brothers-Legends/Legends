this.weapon <- this.inherit("scripts/items/item", {
	...
	function getTooltip()
	{
		...
		if (this.m.WeaponFatalityChanceMult < 1.0)
		{
			result.push({
				id = 6,
				type = "text",
				icon = "ui/icons/chance_to_hit_head.png",
				text = "Has a [color=" + this.Const.UI.Color.NegativeValue + "]" + (1.0 - this.m.WeaponFatalityChanceMult) * 100 + "%[/color] lower threshold to inflict injuries"
			});
		}
	...
	function onBeforeTargetHit( _skill, _target, _hitInfo )
	{
		...
		if (this.m.WeaponFatalityChanceMult < 1.0) { _hitInfo.FatalityChanceMult *= this.m.WeaponFatalityChanceMult; }
	}
